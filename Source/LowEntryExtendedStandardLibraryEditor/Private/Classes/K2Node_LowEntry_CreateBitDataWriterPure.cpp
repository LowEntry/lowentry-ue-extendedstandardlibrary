...
/////////////////////////////////////////////////////
// FKCHandler_LowEntry_CreateBitDataWriterPure

class FKCHandler_LowEntry_CreateBitDataWriterPure : public FNodeHandlingFunctor
{
public:
	FKCHandler_LowEntry_CreateBitDataWriterPure(FKismetCompilerContext& InCompilerContext) : FNodeHandlingFunctor(InCompilerContext)
	{
	}

	virtual void RegisterNets(FKismetFunctionContext& Context, UEdGraphNode* Node) override
	{
		UK2Node_LowEntry_CreateBitDataWriterPure* ArrayNode = CastChecked<UK2Node_LowEntry_CreateBitDataWriterPure>(Node);
	...
	virtual void Compile(FKismetFunctionContext& Context, UEdGraphNode* Node) override
	{
		...
		UFunction* ConvertFunctionPointer = FindField<UFunction>(ConvertFunctionClass, TEXT("BitDataWriter_CreateFromEntryArrayPure"));
...
/////////////////////////////////////////////////////
// UK2Node_LowEntry_CreateBitDataWriterPure
...
FNodeHandlingFunctor* UK2Node_LowEntry_CreateBitDataWriterPure::CreateNodeHandler(FKismetCompilerContext& CompilerContext) const
{
	return new FKCHandler_LowEntry_CreateBitDataWriterPure(CompilerContext);
}

FText UK2Node_LowEntry_CreateBitDataWriterPure::GetNodeTitle(ENodeTitleType::Type TitleType) const
{
	return FText::FromString("Create Bit Data Writer (Pure)");
}

UEdGraphPin* UK2Node_LowEntry_CreateBitDataWriterPure::GetArrayOutputPin() const
{
	return FindPin(TEXT("TempArray"));
}

UEdGraphPin* UK2Node_LowEntry_CreateBitDataWriterPure::GetObjectOutputPin() const
{
	return FindPin(TEXT("BitDataWriter"));
}

bool UK2Node_LowEntry_CreateBitDataWriterPure::IsNodePure() const
{
	return true;
}

void UK2Node_LowEntry_CreateBitDataWriterPure::AllocateDefaultPins()
{
	if(!true)
	{
	...
	UEdGraphPin* ObjectOutputPin = CreatePin(EGPD_Output, UEdGraphSchema_K2::PC_Object, TEXT(""), ULowEntryBitDataWriter::StaticClass(), TEXT("BitDataWriter"));
	FCreatePinParams ArrayOutputPinParams = FCreatePinParams();
	ArrayOutputPinParams.ContainerType = EPinContainerType::Array;
	UEdGraphPin* ArrayOutputPin = CreatePin(EGPD_Output, UEdGraphSchema_K2::PC_Object, TEXT(""), ULowEntryBitDataEntry::StaticClass(), TEXT("TempArray"), ArrayOutputPinParams);
	...
	{
		CreatePin(EGPD_Input, UEdGraphSchema_K2::PC_Object, TEXT(""), ULowEntryBitDataEntry::StaticClass(), *FString::Printf(TEXT("[%d]"), i));
	}
}

void UK2Node_LowEntry_CreateBitDataWriterPure::NotifyPinConnectionListChanged(UEdGraphPin* Pin)
{
...
FText UK2Node_LowEntry_CreateBitDataWriterPure::GetTooltipText() const
{
	return FText::FromString("Creates a new Bit Data Writer.");
}

void UK2Node_LowEntry_CreateBitDataWriterPure::AddInputPin()
{
	FScopedTransaction Transaction(LOCTEXT("AddPinTx", "AddPin"));
	Modify();
...
void UK2Node_LowEntry_CreateBitDataWriterPure::RemoveInputPin(UEdGraphPin* Pin)
{
	FScopedTransaction Transaction(LOCTEXT("RemovePinTx", "RemovePin"));
	Modify();
...
void UK2Node_LowEntry_CreateBitDataWriterPure::GetContextMenuActions(const FGraphNodeContextMenuBuilder& Context) const
{
	...
	if(!Context.bIsDebugging)
	{
		...
		if(Context.Pin != NULL)
		{
			...
			if((Context.Pin->Direction == EGPD_Input) && (Context.Pin->PinType.PinCategory != UEdGraphSchema_K2::PC_Exec))
			{
				...
				Context.MenuBuilder->AddMenuEntry(
					...
					FSlateIcon(),
					FUIAction(
					FExecuteAction::CreateUObject(this, &UK2Node_LowEntry_CreateBitDataWriterPure::RemoveInputPin, const_cast<UEdGraphPin*>(Context.Pin))
					)
					);
			}
		}
		else
		{
			Context.MenuBuilder->AddMenuEntry(
				...
				FSlateIcon(),
				FUIAction(
				FExecuteAction::CreateUObject(this, &UK2Node_LowEntry_CreateBitDataWriterPure::AddInputPin)
				)
				);
		}
...
void UK2Node_LowEntry_CreateBitDataWriterPure::GetMenuActions(FBlueprintActionDatabaseRegistrar& ActionRegistrar) const
{
...
FText UK2Node_LowEntry_CreateBitDataWriterPure::GetMenuCategory() const
{
